...
RSpec.feature "some word" do
  ...
  scenario "can navigate to a spirit's show page" do
    ...
    @spirit = FactoryBot.create(:spirit, name: "Buffalo Trace", user: user)
    visit spirits_path
  ...
  scenario "can create a new spirit" do
    ...
    select user.name, from: 'spirit_user_id'
  ...
  scenario "can delete a spirit" do
    user = FactoryBot.create(:user)
    FactoryBot.create(:spirit, name: "Bulleit", abv: 45, spirit_type: "Bourbon", user: user)
    visit spirits_path
    click_link("Bulleit")
